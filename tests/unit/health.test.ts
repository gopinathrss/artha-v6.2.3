...
function trustFromCheckRows(rows: { status: 'PASS' | 'WARN' | 'FAIL' }[]) {
  ...
  return Math.min(100, Math.round((pass * 100 + warn * 50) / 12))
}

describe('health trust score math', () => {
  it('all PASS → 100', () => {
    const rows = Array.from({ length: 12 }, () => ({ status: 'PASS' as const }))
    expect(trustFromCheckRows(rows)).toBe(100)
  })
  it('all FAIL → 0', () => {
    const rows = Array.from({ length: 12 }, () => ({ status: 'FAIL' as const }))
    expect(trustFromCheckRows(rows)).toBe(0)
  })
  it('6 PASS 6 WARN → 75', () => {
    const rows = [
      ...Array.from({ length: 6 }, () => ({ status: 'PASS' as const })),
      ...Array.from({ length: 6 }, () => ({ status: 'WARN' as const }))
    ]
    expect(trustFromCheckRows(rows)).toBe(75)
  })
  it('HEALTH_CHECK_COUNT is 12', () => {
    expect(HEALTH_CHECK_COUNT).toBe(12)
  })
})
...
describe('runHealthChecks (mocked prisma)', () => {
  it('returns 12 checks and valid trust', async () => {
    const now = new Date()
    ...
    expect(h.checks.length).toBe(12)
    h.checks.forEach((c) => {
